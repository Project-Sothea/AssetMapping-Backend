...
export type EntityType = 'pins' | 'forms';
...
export abstract class BaseOperations<T extends BaseEntity> implements IOperations<T> {
  ...
  abstract getUpdatedAt(id: string): Promise<string | null>;
  ...
  private async deleteEntity(id: string | undefined): Promise<{ id: string; deleted: boolean }> {
    if (!id) {
      throw new Error(`${this.getEntityType()} ID is required for delete operation`);
    }
  ...
  private async resolveVersionConflict(data: T): Promise<void> {
    ...
    logger.info(`Accepting newer update despite version conflict for ${this.getEntityType()}`, {
      ...
      clientTime: new Date(new Date(data.updatedAt || 0).getTime()).toISOString(),
      serverTime: new Date(new Date(serverUpdatedAt).getTime()).toISOString(),
    });
  }
  ...
  private async incrVersionOrSetOne(data: T): Promise<number> {
    ...
    if (data.id) {
      ...
      if (existingVersion !== null) {
        ...
        logger.info(`Creating ${this.getEntityType()} with version 1`, { id: data.id });
      }
    } else {
